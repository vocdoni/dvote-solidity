...
const fs = require("fs")
const { ProcessMode, ProcessEnvelopeType, wrapProcessCreateParams, unwrapProcessState, ProcessStatus } = require("../build/index.js")
...
async function main() {
	...
	// Getters
	const voteDetails = await processInstance1.get(processId)
	console.log("Raw details", voteDetails)
	console.log("Wrapped details", unwrapProcessState(voteDetails))
	...
	const voteDetails2 = await processInstance1.get(processId)
	console.log("Process details from the fork", unwrapProcessState(voteDetails2))
